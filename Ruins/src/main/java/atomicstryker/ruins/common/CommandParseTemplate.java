    ...
    private static void execute(CommandSource source, String input) {
        if (source.getEntity() instanceof PlayerEntity) {
            if (input == null || input.isEmpty()) {
                source.sendFailure(new TranslationTextComponent("You need to use the command with the target template name, eg. /parseruin funhouse"));
            } else {
                player = (PlayerEntity) source.getEntity();
                templateName = input;
                source.sendSuccess(new TranslationTextComponent("Template parser ready to create " + templateName + ". Break any block of the baseplate now."), false);
            }
        } else {
            source.sendFailure(new TranslationTextComponent("Command only available for ingame player entities."));
        }
    }
