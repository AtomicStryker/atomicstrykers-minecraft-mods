...
public class MM_Alchemist extends MobModifier {
    ...
    private void tryAbility(LivingEntity mob, LivingEntity target) {
        ...
        if (mob.getDistanceSq(target) > MIN_DISTANCE) {
            double diffX = target.getPosX() + target.getMotion().x - mob.getPosX();
            double diffY = target.getPosY() + (double) target.getEyeHeight() - 1.100000023841858D - mob.getPosY();
            double diffZ = target.getPosZ() + target.getMotion().z - mob.getPosZ();
            ...
            mob.world.playSound(null, mob.getPosX(), mob.getPosY(), mob.getPosZ(), SoundEvents.ENTITY_WITCH_THROW, mob.getSoundCategory(), 1.0F, 0.8F + mob.world.rand.nextFloat() * 0.4F);
