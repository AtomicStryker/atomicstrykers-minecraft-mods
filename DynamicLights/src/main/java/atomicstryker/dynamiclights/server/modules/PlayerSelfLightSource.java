...
public class PlayerSelfLightSource {
    ...
    public void playerTick(PlayerTickEvent.Post event) {
        ...
        if (player.isAlive()) {
            ...
            boolean isUnderwater = checkPlayerWater(event.getEntity());
            ...
            for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
                ItemStack equippedStack = event.getEntity().getItemBySlot(equipmentSlot);
                int itemLight = getLightFromItemStack(equippedStack, isUnderwater, event.getEntity().level().registryAccess());
    ...
    private int getLightFromItemStack(ItemStack stack, boolean isUnderwater, RegistryAccess registryAccess) {
        ...
        if (isUnderwater && (notWaterProofItems.getLightLevel(stack, registryAccess) > 0 || stack.tags().anyMatch(rl -> rl.location().equals(DynamicLights.NOT_WATERPROOF_TAG)))) {
            return 0;
        }
